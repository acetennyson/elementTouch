...
export default class DataTable {
    ...
    constructor(param){
        this.param = param
        this.asc = param.asc || false;
        this.orderby = param.orderby || null;
        this.searchText = param.search || null;
        this.lastRequest = 0;
        this.searchDelay = param.searchDelay || Math.random() * 800 + 500;
    }
    createDataTable(params){
        this.selector = params.selector;
        this.setTable();
        this.data = params.data || this.data || [];
        this.columns = params.columns || this.columns || Array.from(this.table.querySelectorAll('thead tr th')).map(th => ({
            title: th.innerText,
            id: th.id,
            class: th.className,
        ...
        this.rowsPerPage = params.size || this.rowsPerPage || 10;
        this.data = param.data || this.data;
        ...
        this.bDestroy = params.bDestroy !== undefined ? params.bDestroy : (this.bDestroy || false);
        this.columnDefs = params.columnDefs || this.columnDefs || [];
        this.responsive = params.responsive !== undefined ? params.responsive : (this.responsive || true) ;
        this.searching = params.searching !== undefined ? params.searching : (this.searching || true);
        this.info = params.info !== undefined ? params.info : true;
        this.paging = params.paging !== undefined ? params.paging : (this.paging || true);
        ...
        this.loadedPages = Math.ceil(this.data.length || 0 / this.rowsPerPage)
    ...
    updateDataTable(params){
        ...
        this.step = params.step || this.step || 0;
        this.parent.size = params.rowsPerPage || this.size;
        this.data = params.data || this.data;
        ...
        this.loadedPages = Math.ceil(this.data.length / this.rowsPerPage)
    ...
    renderTableHead(){
        ...
        this.columns.forEach((column, index) => {
            ...
            if(column.data) th.addEventListener('click', ()=>{
                ...
                this.asc = (column.data==this.orderby)?!this.asc:true;
                this.orderby = column.data;
                this.eventbus.dispatch('sort', this.orderby, this.asc)
                // this.sendRequest();
            })
    ...
    renderTableBody2() {
        ...
        const paginatedData = this.data.slice(startIndex, startIndex + this.rowsPerPage);
    ...
    addPaginationControls() {
        ...
        prevButton.firstChild.addEventListener('click', ()=>{
            ...
            // dont forget to update this.step
        ...
        this.loadedPages = this.totalPages?this.totalPages:Math.ceil((this.parent.data.length || 1) / this.size);
        const pageButton = document.createElement('li');
        pageButton.classList.add('page-item', 'flex');
        pageButton.textContent = `${this.step + 1}/${this.loadedPages}`;
