...
export const serializeForm = function (form) {
    ...
    for (let i = 0; i < inputs.length; i++) {
        ...
        if (!name) throw `name attribute needed for input ${element.id}`;
    ...
    for (let i = 0; i < textareas.length; i++) {
        ...
        let name = element.attributes.getNamedItem("name");
        if (!name) throw `name attribute needed for textarea ${element.id}`;
        result += name.value + "=" + element.value + "&";
        obj[name.value] = element.value;
    }
    ...
    for (let i = 0; i < selects.length; i++) {
        ...
        let name = element.attributes.getNamedItem("name");
        if (!name) throw `name attribute needed for select ${element.id}`;
        result += name.value + "=" + element.value + "&";
        obj[name.value] = element.value;
    }
